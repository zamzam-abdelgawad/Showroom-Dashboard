      ...
      <Card>
        ...
        <CardContent className="p-0">
            ...
            <table className="min-w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? (
                  ...
                  <tr><td colSpan="5" className="px-6 py-12 text-center text-gray-500"><div className="flex flex-col items-center"><Search className="h-10 w-10 text-gray-300 mb-2" /><p>No cars found matching your criteria</p></div></td></tr>
                ) : (
                  paginatedCars.map((car) => (
                    <tr key={car.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${car.status === 'Available' ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-600'}`}>{car.status}</span>
                      </td>
          ...
          <div className="px-6 py-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
            <span className="text-sm text-gray-500">Showing <span className="font-medium">{filteredCars.length === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1}</span> to <span className="font-medium">{Math.min(currentPage * itemsPerPage, filteredCars.length)}</span> of <span className="font-medium">{filteredCars.length}</span> results</span>
            <div className="flex items-center gap-2">
