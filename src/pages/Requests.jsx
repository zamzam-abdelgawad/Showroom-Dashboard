        ...
        <CardHeader className="bg-gray-50/50 border-b border-gray-100 flex flex-col sm:flex-row gap-4 justify-between pb-4 pt-6 px-6">
          <div className="relative max-w-sm w-full">
            ...
            <input 
              ...
              className="pl-10 h-10 w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
              value={searchTerm}
            ...
            <table className="min-w-full text-sm text-left">
              <thead className="bg-gray-50/80 text-gray-400 font-bold uppercase tracking-wider text-[11px] border-b border-gray-100">
                <tr>
                  ...
                  <th className="px-6 py-4">Vehicle Requested</th>
                  ...
                  <tr>
                    <td colSpan="5" className="px-6 py-16 text-center text-gray-400">
                      <div className="flex flex-col items-center">
                        <Clock className="h-12 w-12 text-gray-100 mb-2" />
                        <p className="text-gray-500 font-medium tracking-tight">No requests found matching your filters.</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={req.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="font-bold text-gray-900 mb-0.5">{req.userName}</span>
                          <span className="text-xs text-gray-400 flex items-center gap-1"><Mail className="h-3 w-3" /> {req.userEmail}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="font-medium text-indigo-600 hover:underline cursor-pointer flex items-center gap-1.5" onClick={() => navigate(`/cars/${req.carId}`)}>
                            <Car className="h-3.5 w-3.5" /> {req.carName}
                          </span>
                          <span className="text-[11px] text-gray-400 font-medium uppercase tracking-tighter">{req.carBrand}</span>
                        </div>
                      </td>
                        ...
                        {req.status === 'pending' ? (
                          <div className="flex justify-end gap-2">
                            <Button 
                              variant="ghost" 
                              size="sm" 
                              className="text-green-600 hover:bg-green-50 h-8 font-bold"
                              onClick={() => handleStatusChange(req.id, 'approved', req.carId)}
                            >
                              Approve
                            </Button>
                            <Button 
                              variant="ghost" 
                              size="sm" 
                              className="text-red-600 hover:bg-red-50 h-8 font-bold"
                              onClick={() => handleStatusChange(req.id, 'rejected', req.carId)}
                            >
                              Reject
                            </Button>
                          </div>
